// scripts/updateTokensMetadata.js
 ...
 */
async function updateToken(tokenInfo, availableColumns) {
  const { token_address, symbol, name, metadata, blockchain, standard } = tokenInfo;
  
  if (!token_address) {
    console.error('Error: token_address es requerido para actualizar un token');
    return;
  }
  ...
  try {
    ...
    if (existingToken) {
      ...
      // El token no existe, considerar insertarlo
      console.log(`Token ${token_address} no encontrado en la base de datos. ¿Deseas insertarlo?`);
      
      // Aquí podrías añadir lógica para preguntar al usuario o insertar automáticamente
      // Por ahora, solo mostramos un mensaje
      console.log('Para insertar tokens faltantes, considera usar el script syncTokens.js');
    }
  } catch (error) {
...
/**
 * Actualiza la información de todos los tokens
 */
async function updateAllTokens() {
  console.log('Iniciando actualización de información de tokens...');
  ...
  if (!availableColumns.includes('metadata')) {
    console.log('La columna "metadata" no existe en la tabla. ¿Deseas crearla? (Por ahora continuamos sin ella)');
    // Aquí podrías añadir código para crear la columna si es necesario
  }
  
  // Procesar cada token
  for (const tokenInfo of TOKENS_TO_UPDATE) {
    await updateToken(tokenInfo, availableColumns);
  }
